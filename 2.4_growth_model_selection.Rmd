...
dir.create(here("data", "models", "dredge_tables"), recursive = TRUE, showWarnings = FALSE)
```
...
## Multi-model selection: planting-anchored (spring) candidate windows

`na.action` is set once per chunk rather than before every model, and restored
to R's default at the very end of the second chunk. Each chunk's cluster is
explicitly stopped when that chunk's dredging is finished.
...
# Load packages to be used -- library()'s 2nd argument is `help`, not a 2nd
# package, so both packages need their own call to actually load on the workers
clusterEvalQ(clu, {library(MuMIn); library(stats)})
...
## Multi-model selection: July-anchored (summer/fall) candidate windows
