...
planting_survival <- planting %>% 
  select(Plant, Height, Date.Planted) %>% 
...
planting_growth <- planting %>% 
  ...
  filter(Plant != "No Plant", Height != "") %>% 
  mutate(date = my(0320)) %>% 
  select(Plant, Height, Width.1, Width.2, Stem.Diameter, date)
...
watering_gopher <- watering %>% 
  select(ID, Watering.2.Date, Alive.1, Notes.1) %>% 
...
survival_gopher <- right_join(planting, watering_gopher, by = "ID") %>% 
  right_join(measuring_gopher, by = "ID") %>% 
  right_join(measuring_gopher_2, by = "ID") %>% 
  right_join(measuring_gopher_3, by = "ID") %>%  
...
watering_survival_gopher <- survival_gopher %>%
  select(Plant, watering_alive, watering_date) %>% 
  mutate(Plant = case_when(Plant == "ENCA (Pink)" ~ "ENCA",
                           Plant == "CEOL (Orange)" ~ "CEOL",
                           Plant == "HEWH (White)" ~ "HEWH",
                           Plant == "SAME (Orange + Pink)" ~ "SAME",
...
measuring_survival_gopher <- survival_gopher %>% 
  select(Plant, measuring_height, measuring_date) %>% 
  mutate(Plant = case_when(Plant == "ENCA (Pink)" ~ "ENCA",
                           Plant == "CEOL (Orange)" ~ "CEOL",
                           Plant == "HEWH (White)" ~ "HEWH",
                           Plant == "SAME (Orange + Pink)" ~ "SAME",
...
measuring_survival_gopher_2 <- survival_gopher %>% 
  select(Plant, measuring_2_height, measuring_2_date) %>% 
  mutate(Plant = str_replace(Plant, " ", ""),
...
measuring_survival_gopher_3 <- survival_gopher %>% 
  select(Plant, measuring_3_height, measuring_3_date) %>% 
  mutate(Plant = case_when(Plant == "ENCA (Pink)" ~ "ENCA",
                           Plant == "CEOL (Orange)" ~ "CEOL",
                           Plant == "HEWH (White)" ~ "HEWH",
                           Plant == "SAME (Orange + Pink)" ~ "SAME",
                           ...
                           Plant == "NO PLANT" ~ "No Plant",
                           Plant == "HEWH (White) RHOV" ~ "RHOV",
                           Plant == "HEWH" ~ "HEWH"),alive = case_when(measuring_3_height > 0 ~ 1),
        date = my(1221)) %>% 
  filter(Plant != "") %>% 
  select(Plant, alive, date)
