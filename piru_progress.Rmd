...
## Next steps: 
 # read in new soil moisture data and re-run regressions. 
...
## Make three lms per month (PC1, PC2, PC1 + PC2)
## Use AIC to pick best model per month. 
```{r}
...
```{r}
my_files <- list.files(here("data", "Ibutton_Data", "test"), pattern = ".csv$", recursive = TRUE, full.names = TRUE)
test <- read_csv(my_files, id = "plot") %>%  
  mutate(plot = str_sub(plot, start = -8),
         plot = str_remove(plot, ".csv"),
         plot = str_remove(plot, "t"),
         plot = str_remove(plot, "s"),
         plot = str_remove(plot, "/"),
         plot = as.numeric(plot),
         date_time = mdy_hm(`Date/Time`),
         temperature = as.numeric(Value)) %>% 
  dplyr::select(plot, date_time, temperature)

ggplot(test, aes(x = date_time, y = temperature, col = as.factor(plot))) +
  geom_point()
