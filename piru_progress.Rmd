...
topography_gopher_july <- topography_gopher_survival %>% 
  ...
  filter(date == "2020-07-01" | alive == 1)
  filter(date == "2020-0701")
```

## Binomial regression for may survival

```{r}
watering_binomial <- watering_survival_gopher_topography %>% 
...
```{r}
july_binomial <- measuring_survival_gopher_topography %>% 
  dplyr::select(!plot_number & !date & !geometry)  
