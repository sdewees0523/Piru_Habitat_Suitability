---
title: "Modeling environmental variables across the"
author: "Shane Dewees"
date: "2022-09-28"
...
aspect <- mask(aspect, elevation)
slope <- rast(here("data", "raw_data", "slope.tif"))
slope <- mask(slope, elevation)
solar_radiation <- rast(here("data", "raw_data", "sol_rad.tif"))
solar_radiation <- mask(solar_radiation, elevation)
...
plots_topography <- st_read(here("data", "raw_data", "plot_gps_points.shp")) %>% 
  ...
  st_as_sf() |> 
  rename(summer_solar_radiation = sol_rad_1,
  spring_solar_radiation = sol_rad_2,
  winter_solar_radiation = sol_rad_3,
  heat_load = lyr.1,
  diurnal_heat = aspect.1,
  topo_position = elevation.1,
  sol_rad_aspect = lyr.1.1,
  terrain_ruggedness = elevation.2,
  saga_wetness = elevation.3,
  twi = elevation.4) |> 
    filter(elevation != 0)

st_write(plots_topography, here("data", "clean_data", "plots_topography.shp"), append = FALSE)
```
...
rmse(soil_predict$soil_moisture, soil_predict$predicted)
...
set.seed(123)
temp_rf <- randomForest(temperature ~ ., data = temp_train, importance = TRUE, mtry = 5)
temp_rf
temp_rf[["importance"]]
...
rmse(temp_predict$temperature, temp_predict$predicted)
...
rmse(vpd_predict$vpd, vpd_predict$predicted)
...
write.csv(predicted_environmental, here("data", "clean_data", "predicted_environmental.csv"), row.names = FALSE)
```