---
title: "Community and Species hazards models"
author: "Shane Dewees"
...
first_year_glht <- glht(first_year_hazard_model, linfct = multcomp::mcp(Plant = "Tukey"))
first_year_letters <- cld(first_year_glht) 
first_year_letters_table <- as.data.frame(first_year_letters$mcletters$Letters) |> 
  rownames_to_column(var = "species") |> 
  rename(letters = "first_year_letters$mcletters$Letters")
...
whole_study_glht <- glht(whole_study_hazard_model, linfct = multcomp::mcp(Plant = "Tukey"))
whole_study_letters <- cld(whole_study_glht) 
whole_study_letters_table <- as.data.frame(whole_study_letters$mcletters$Letters) |> 
  rownames_to_column(var = "species") |> 
  rename(letters = "whole_study_letters$mcletters$Letters")
...
first_year_species_survival <- first_year_hazard[["table_body"]]$label %>% 
  ...
  filter(species != "HEWH") %>% 
  left_join(first_year_letters_table, by = "species")%>%
...
figure2 <- community_survival/(first_year_hazard_graph + whole_study_hazard_graph) + plot_annotation(tag_levels = "a")
