...
```{r}
load(here("data", "models", "summer_growth_model.rda"))
summary(summer_growth_model)

summer_wood <- tibble(wood_density = seq(from = min(growth_summer_data$wood_density), 
                                       to = max(growth_summer_data$wood_density), 
                                       by = 0.1)) %>% 
  mutate(soil_moisture = median(growth_summer_data$soil_moisture),
         temperature = median(growth_summer_data$temperature),
         les_2 = median(growth_summer_data$les_2),
         wue = median(growth_summer_data$wue)) %>% 
  mutate(growth = predict(summer_growth_model, newdata = .),
         interval = predict(summer_growth_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
         upper = growth + interval,
         lower = growth - interval)

summer_temperature <- expand.grid(temperature = seq(from = min(growth_summer_data$temperature), 
  ...
  mutate(wood_density = median(growth_summer_data$wood_density),
         soil_moisture = median(growth_summer_data$soil_moisture),
         wue = median(growth_summer_data$wue)) %>% 
  mutate(growth = predict(summer_growth_model, newdata = .),
         interval = predict(summer_growth_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
         upper = growth + interval,
...
```{r}
summer_les2_graph <- ggplot(summer_temperature, aes(x = temperature, y = growth, col = as.factor(les_2)))+
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
  theme_classic()+
  scale_color_manual(values = c("#648FFF", "#DC267F", "#FFB000"))+
...
figure_9 <- (summer_wood_graph + summer_wue_graph)/(summer_les2_graph + plot_spacer()) + plot_annotation(tag_levels = "a")
