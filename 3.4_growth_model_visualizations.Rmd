...
environmental_late_spring <- environmental %>% 
...
growth_spring_data <- spring_height %>% 
  ...
  inner_join(environmental_late_spring, by = "plot_number") %>% 
...
```{r}
load(here("data", "models", "late_spring_growth_model.rda"))
...
spring_planting_height <- expand.grid(planting_height = seq(from = min(growth_spring_data$planting_height), to = max(growth_spring_data$planting_height), by = 0.1),
  ...
  mutate(growth = predict(late_spring_model, newdata = . ),
         interval = predict(late_spring_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
...
spring_les2_graph <- ggplot(spring_les2, aes(x = les_2, y = growth))+
  ...
  geom_text(data = plant_traits, aes(x= les_2, y = 0.05, label = species),
            vjust = c(0,-3,-4,-1,3,0.5,-0.5,-2,1),
            hjust= c(0,0,0,0,0.75,0,0,0,0),
            size = 5)+
  theme_classic()+
  geom_hline(yintercept = 0, linetype = "dashed")+
  labs(x = "LES 2",
...
figure_8 <- (spring_height_graph+ spring_les2_graph) + plot_annotation(tag_levels = "a")
...
```{r}
load(here("data", "models", "late_summer_growth_model.rda"))
summary(late_summer_model)

summer_wood <- tibble(wood_density = seq(from = min(growth_summer_data$wood_density), 
                                       to = max(growth_summer_data$wood_density), 
                                       by = 0.1)) %>% 
  mutate(temperature = median(growth_summer_data$temperature),
         les_2 = median(growth_summer_data$les_2),
         wue = median(growth_summer_data$wue)) %>% 
  mutate(growth = predict(late_summer_model, newdata = .),
         interval = predict(late_summer_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
         upper = growth + interval,
         lower = growth - interval)

summer_temperature <- tibble(temperature = seq(from = min(growth_summer_data$temperature), 
                                                  to = max(growth_summer_data$temperature), 
                                                  by = 0.1)) %>% 
  mutate(les_2 = median(plant_traits$les_2),
         wood_density = median(growth_summer_data$wood_density),
         wue = median(growth_summer_data$wue)) %>% 
  mutate(growth = predict(late_summer_model, newdata = .),
         interval = predict(late_summer_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
         upper = growth + interval,
...
summer_wue <- tibble(wue = seq(from = min(plant_traits$wue),
                             to = max(plant_traits$wue),
                             by = 0.1)) %>% 
  mutate(les_2 = median(growth_summer_data$les_2),
         wood_density = median(growth_summer_data$wood_density),
         temperature = median(growth_summer_data$temperature)) %>% 
  mutate(growth = predict(late_summer_model, newdata = .),
         interval = predict(late_summer_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
         upper = growth + interval,
         lower = growth - interval,
...
summer_les2 <- tibble(les_2 = seq(from = min(growth_summer_data$les_2),
                               to = max(growth_summer_data$les_2),
                               by = 0.1)) %>% 
  mutate(wood_density = median(growth_summer_data$wood_density),
         temperature = median(growth_summer_data$temperature),
         wue = median(growth_summer_data$wue)) %>% 
  mutate(growth = predict(late_summer_model, newdata = .),
         interval = predict(late_summer_model, newdata = ., se.fit = T, interval = "confidence")[[2]],
...
```{r}
summer_les2_graph <- ggplot(summer_les2, aes(x = les_2, y = growth))+
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
  geom_text(data = plant_traits, aes(x = les_2, y = 0.05, label = species),
            hjust= c(0,0,0,0,2,0,0,0,0),
            vjust = c(7,3.5,2,0,-8,-2,0,-2,6))+
  theme_classic()+
  geom_hline(yintercept = 0, linetype = "dashed")+
  ylim(-.2, 0.35)+
  labs(x = "LES 2",
...
summer_wood_graph <- ggplot(summer_wood, aes(x = wood_density, y = growth))+
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
...
summer_wue_graph <- ggplot(summer_wue, aes(x = wue, y = growth))+
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
...
summer_temperature_graph <- ggplot(summer_temperature, aes(x = temperature, y = growth))+
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
  theme_classic()+
  geom_hline(yintercept = 0, linetype = "dashed")+
...
figure_9 <- (summer_wood_graph + summer_wue_graph)/(summer_les2_graph + summer_temperature_graph) + plot_annotation(tag_levels = "a")
